...
CREATE INDEX IF NOT EXISTS idx_grab_t_retail_bids_bid_id ON grab_t_retail_bids(bid_id);
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'retail_bids') THEN
    INSERT INTO grab_t_retail_bids (bid_id, user_id, flight_id, submitted_amount, passenger_count, status, created_at, updated_at)
    ...
    WHERE NOT EXISTS (
      ...
      WHERE grb.bid_id = retail_bids.bid_id 

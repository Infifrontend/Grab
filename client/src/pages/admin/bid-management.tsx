  ...
  const renderActiveBidsContent = () => {
    ...
    const activeBids = (recentBidsData || []).map((bid, index) => {
      ...
      return {
        ...
        paymentStatus:
          bid.bidStatus === "completed" || bid.rStatus === 3
            ? "paid"
            : "pending", // Mock payment status
          ...
          <Table
            ...
            expandable={{
              expandedRowRender: (record: any) => {
                ...
                if (
                  !retailData ||
                  !retailData.retailUsers ||
                  retailData.retailUsers.length === 0
                ) {
                  return (
                    <div className="bg-gray-50 p-4 rounded-lg text-center">
                      <Text className="text-gray-500">
                              ...
                              {user.status === "pending_approval" && (
                                <>
                                  <Button
                                    ...
                                    onClick={() => {
                                      console.log(
                                        `Approving retail bid ID: ${user.rUserId} for user: ${user.userId} on bid: ${record.bidId}`,
                                      );
                                      console.log("User object:", user);
                                      // user.rUserId now contains the grab_t_retail_bids.id
                                      // user.userId contains the actual user ID
                                      handleRetailUserAction(
                                        user.rUserId, // This is grab_t_retail_bids.id
                                        user.userId,  // This is the actual user ID
                                        "approve",
                                        record.bidId,
                                      );
                                    }}
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      console.log(
                                        `Rejecting retail bid ID: ${user.rUserId} for user: ${user.userId} on bid: ${record.bidId}`,
                                      );
                                      console.log("User object:", user);
                                      // user.rUserId now contains the grab_t_retail_bids.id
                                      // user.userId contains the actual user ID
        ...
        const transformedData = {
          ...
          retailUsers: retailUsers.map((user, index) => {
            ...
            return {
              ...
              seatNumber:
                user.seatNumber ||
                `1${2 + index}${String.fromCharCode(65 + (index % 26))}`,
              bidAmount:

 ...
 */
export const getHmacEncyption = (data: string) => {
  return CryptoJS.HmacSHA256(data, CFG.encryption.encryption_key).toString();
};
...
export const decryptData = (encryptedData: string): string => {
  ...
  // AES decryption
  const bytes = CryptoJS.AES.decrypt(encryptedString, CFG.encryption.encryption_key);

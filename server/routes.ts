...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin booking update endpoint
  app.put("/api/admin/booking-update/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const {
        bookingStatus,
      ...
      // Update comprehensive data with new information
      if (groupLeaderInfo) {
      ...
      await storage.updateBookingDetails(booking.id, updateData);
